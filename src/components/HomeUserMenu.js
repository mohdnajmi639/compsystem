...
export default function HomeUserMenu({ session }) {
  ...
  const roleLabel =
    role === 'admin' ? 'Pentadbir' : role === 'staff' ? 'Staf' : role;
    ...
    <div className="db-topbar-user-wrap" style={{ marginLeft: '12px' }}>
      ...
      >
        <div
          ...
          style={{
            background: 'linear-gradient(135deg, #8b2be2, #5b21b6)',
            color: '#fff',
            borderRadius: 0,
          }}
        >
        ...
        <div className="db-topbar-user-text">
          <span className="db-topbar-user-name" style={{ color: '#111827' }}>
            {session?.user?.name?.split(' ')[0]}
          </span>
          <span className="db-topbar-user-role" style={{ color: '#6b7280' }}>
            {roleLabel}
          </span>
        </div>
        <svg
        ...
        <>
          ...
          >
            ...
            {(role === 'admin' || role === 'staff') && (
              <Link
                href="/dashboard"
                className="db-topbar-dropdown-item"
                onClick={() => setUserMenuOpen(false)}
              >
                <svg
                  width="13"
            ...
            <button
              className="db-topbar-dropdown-item db-topbar-dropdown-logout"
