      ...
      <div className="page-content">
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px', borderBottom: '1px solid #e5e7eb', background: '#f9fafb' }}>
            <div>
              <h3 style={{ fontSize: '1rem', color: '#111827', margin: 0, fontWeight: 700 }}>Peti Masuk</h3>
              <p style={{ color: '#6b7280', fontSize: '0.8rem', margin: '4px 0 0 0' }}>
                {notifications.filter(n => !n.read).length} belum dibaca
              </p>
            </div>
            <button className="btn btn-secondary btn-sm" onClick={markAllRead}>Tanda semua sebagai dibaca</button>
          </div>
          ...
          {loading ? (
            ...
            <div className="empty-state" style={{ padding: '60px 20px' }}>
              ...
              <h3 style={{ color: '#6b7280', margin: '0 0 8px 0', fontSize: '1rem' }}>Tiada pemberitahuan</h3>
              <p style={{ margin: 0, fontSize: '0.9rem' }}>Anda telah membaca semua pemberitahuan!</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {notifications.map((n, idx) => (
                <Link key={n._id} href={n.relatedComplaint ? `/dashboard/complaints/${n.relatedComplaint._id || n.relatedComplaint}` : '#'} style={{ textDecoration: 'none', color: 'inherit' }}>
                  <div style={{ 
                    display: 'flex', gap: '14px', alignItems: 'flex-start', padding: '20px 24px', 
                    borderBottom: idx === notifications.length - 1 ? 'none' : '1px solid #e5e7eb',
                    background: n.read ? '#ffffff' : '#fafafe',
                    transition: 'background 0.2s',
                    position: 'relative'
                  }}>
                    {!n.read && (
                      <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '3px', background: '#7c3aed' }} />
                    )}
                    <div style={{ fontSize: '1.2rem', marginTop: '2px' }}>
                      {typeIcons[n.type] || '🔔'}
                    </div>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: '0.92rem', color: '#111827', marginBottom: '4px' }}>
                        {n.title}
                      </div>
